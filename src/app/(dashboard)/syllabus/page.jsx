    ...
    try {
      ...
      if (missingLevels.length === 0) {
        ...
        missingLevels.forEach((item) => {
          ...
          y = addText(
            ...
            margin + 5,
            y,
            { isListItem: true, fontSize: 12, color: [220, 53, 69], isBold: true }
          );
          y = addText(
            `Reason: ${reason}`,
            margin + 10,
      ...
      if (misalignments.length === 0) {
        ...
        misalignments.forEach((item) => {
          ...
          y = addText(misalignmentText, margin + 5, y, { isListItem: true });
        });
      }
      y += 10;
      ...
      if (suggestions.length === 0) {
        ...
        suggestions.forEach((item, index) => {
          ...
          y = addText(`${index + 1}. ${suggestionText}`, margin + 5, y, { color: [0, 100, 0] });
        });
      }
      y += 15;
      ...
      const missingLevelsText = missingLevels.length === 0 
        ? "All Bloom's taxonomy levels covered."
        : missingLevels.map((item) => {
            const level = typeof item === 'string' ? item : item.level;
            const reason = typeof item === 'string' ? 'Incorporate this cognitive level for better learning outcomes.' : item.reason;
            const impact = typeof item === 'string' ? 'Limits comprehensive learning outcomes.' : item.impact;
            const recommendation = typeof item === 'string' ? 'Add objectives using appropriate action verbs.' : item.recommendation;
                              ...
                              return (
                                <div 
                                  ...
                                  className={`flex items-center p-3 rounded-lg border ${
                                    isFound ? 'bg-green-50 dark:bg-green-900/20 border-green-500' : 'bg-red-50 dark:bg-red-900/20 border-red-500'
                                  }`}
                                >
                                  <div className="flex-1">
                                    <span className="font-medium text-gray-800 dark:text-gray-200">{level}</span>
                                    <span className={`ml-2 text-sm ${
                                      isFound ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                                    }`}>
                        ...
                        <div>
                          ...
                          {backendResponse?.enhancement?.bloom_analysis?.missing_levels?.length === 0 ? (
                            ...
                            <div className="space-y-4 mt-2">
                              {backendResponse?.enhancement?.bloom_analysis?.missing_levels?.map((item, index) => {
                                const level = typeof item === 'string' ? item : item.level;
                                ...
                                return (
                                  <div key={index} className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border-l-4 border-red-500">
                                    <p className="text-base font-medium text-gray-800 dark:text-gray-200">{level}</p>
                                    <div className="ml-4 text-sm text-gray-700 dark:text-gray-300 space-y-1">
                                      <p><span className="font-medium">Reason:</span> {reason}</p>
                                      <p><span className="font-medium">Impact:</span> {impact}</p>
                                      <p><span className="font-medium">Recommendation:</span> {recommendation}</p>
                                    </div>
                                  </div>
                                );
                              })}
                            ...
                            <div className="space-y-4 mt-2">
                              {backendResponse?.enhancement?.suggestions?.map((item, index) => (
                                <div key={index} className="flex items-start p-3 bg-green-50 dark:bg-green-900/10 rounded-lg">
                                  <span className="bg-green-600 text-white font-bold rounded-full w-8 h-8 flex items-center justify-center mr-3 flex-shrink-0">
                                    {index + 1}
                                  </span>
                                  <div>
